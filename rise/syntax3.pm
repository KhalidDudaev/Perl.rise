...
########################################################################################
	...
	rule _function_callmember			    => q/(_NOT:\#)<name>(NOT:\#)/;
	...
	action _function_callmember				=> \&_syntax_function_callmember;
...
sub _syntax_function_callmember {
    my ($self, $rule_name, $confs)			= @_;
	my $parent_class       = $confs->{parent};
	# my $parent_type     = $confs->{parent_type}||'';
	my $name               = &name;
    ...
    foreach my $n ($name_full =~ m/(\w+)/gsx){
        $name_selected      .= $n;
        $members_list       .= ' ' . join ' ', (&var_members->{$name_selected}||'') =~ m/function-(\w+)/gsx;
        $name_selected      .= '::';
    }
