package rise::core::object::class::helper2;
use strict;
use warnings;
use utf8;
...
sub new {
    ...
    map { no strict; no warnings;
        ...
        # my $native_name = $memberPref . $m_name;
        ...
        *{$class.'::'.$m_name} = sub ():lvalue {  my $self = shift; $self->{$m_name} } if $m_type eq 'var' && $m_accmod eq 'public'; # var instancing
        ...
        # *{$class.'::'.$m_name} = *{$class.'::'.$native_name};
        *{$class.'::'.$m_name} = *{$class.'::'.$memberPref.$m_name} if $m_type eq 'function' && $m_accmod eq 'public'; # var instancing
        # *{$class.'::NATIVE::'.$m_name} = \&{$m_name} if $m_type eq 'function' && $m_accmod eq 'protected'; # var instancing
        # *{$self.'::NATIVE::'.$m_name} = *{$class.'::'.$m_name} if $m_type eq 'function' && $m_accmod eq 'private'; # var instancing
        # if ($m_type eq 'function' && $m_accmod eq 'private') {
        #     # say "### PRIVATE";
        #     *{$class.'::'.$m_name} = *{$class.'::NATIVE_'.$m_name};
        #     # delete ${$class.'::'}{$m_name};
        # }

        # *{$class.'::'.$m_name} = \&{$class.'::NATIVE::'.$m_name};
    ...
    # $self->{'native'} = $class.'::NATIVE';
    # $self->{'native'} = bless {}, $class.'::NATIVE';
...
sub import { no strict "refs";
    ...
    foreach my $parent (@{"$self\:\:ISA"}) {
        # print "### $parent ###\n";

        map { no strict;
            my ($m_accmod, $m_type, $m_name, $m_cast, $m_cast_args)	= $_ =~ m/(\w+)\-(\w+)\-(\w+)(?:\s*\:\s*(\w+)(?:\((.*?)\))?)?/;

            # print "MAP ### $m_accmod - $m_type - $m_name ###\n";
            # // FIXME  не работает кастинг при наследовании
            # __PACKAGE__->__RISE_CAST($m_type, \$parent->__RISE_SELF__->{$m_name}, $m_type_args) if $m_type;
            # $child->{$m_name} = $parent->__RISE_SELF__->{$m_name} if $m_name &&  exists &{$parent.'::__RISE_SELF__'};

            *{$parent.'::'.$m_name} = sub ():lvalue {  my $self = shift; $self->{$m_name} } if $m_type eq 'var' && $m_accmod eq 'public'; # var instancing

            *{$parent.'::'.$m_name} = *{$parent.'::'.$memberPref.$m_name} if $m_type eq 'function' && $m_accmod eq 'public'; # var instancing
            *{$parent.'::'.$m_name} = *{$parent.'::'.$memberPref.$m_name} if $m_type eq 'function' && $m_accmod eq 'protected'; # var instancing

        } split( /\s+/, $parent->__CLASS_MEMBERS__ ) if exists &{ $parent.'::__CLASS_MEMBERS__' };
...
sub export { no strict "refs"; no warnings;
	...
	foreach my $func (@$exports){
        $code = sub { &{$self.'::'.$memberPref.$func}($self->__RISE_SELF__, @_) };
