...
{ package rise::lib::web; use rise::core::object::namespace;   
    { package rise::lib::web::template; use rise::core::object::class;  our $AUTHORITY = "unknown"; sub AUTHORITY {"unknown"}; our $VERSION = "2016.1105052847"; sub VERSION {"2016.1105052847"}; my $__RISE_SELF__ = bless {}; sub __RISE_SELF__ ():lvalue { $__RISE_SELF__ } sub __CLASS_MEMBERS__ {q{export:simple-function-template public-var-self public-var-tname public-var-params  public-function-compile  private-function-tag  private-var-thtml  private-var-file}} sub __EXPORT__ { {":all"=>[qw/wsay out env req template/],":function"=>[qw/wsay out env req template/],":simple"=>[qw/wsay out env req template/],"template"=>[qw/template/],} } 
        ...
        { package rise::lib::web::template::compile; use rise::core::object::function;  sub compile {  my $self; no warnings; sub self ():lvalue; *self = sub ():lvalue { $self }; use warnings;  $self = $_[0]; my $tname; no warnings; sub tname ():lvalue; *tname = sub ():lvalue { $tname }; use warnings; my $params; no warnings; sub params ():lvalue; *params = sub ():lvalue { $params }; use warnings;  ($tname,$params) = ($_[1],$_[2]); 
            # self.dmsg(tname);
        ...
        { package rise::lib::web::template::tag; use rise::core::object::function; sub tag { __PACKAGE__->__RISE_ERR('CODE_PRIVATE', 'tag') unless (caller eq 'rise::lib::web::template' || caller =~ m/^rise::lib::web::template\b/o); my $self; no warnings; sub self ():lvalue; *self = sub ():lvalue { $self }; use warnings;  $self = $_[0]; my $str; no warnings; sub str ():lvalue; *str = sub ():lvalue { $str }; use warnings;  ($str) = ($_[1]); 
            # str =~ s{\>\s*\<}{\>\.\<}gsx;
            $str =~ s{(\<\/?\w+\>)}{\'$1\'}gsx;
            # str =~ s{[^\'](\<\/?\w+\>)[^\']}{\'$1\'}gsx;
            return $str;
